 ...
 * @LastEditTime: 2022-04-18 21:31:56
...
class PlaylistSongsController extends GetxController {
  ...
  Future<void> onSonglistItemClick(Playlist p, int index) async {
    ...
    if (p.id == audioController.currentPlaylist.id) {
      audioHandler.skipToQueueItem(index).then((value) => audioHandler.play());
    } else {
      //更新当前的歌单
      audioController.updatePlaylist(
          playlistState.currentPlaylist.value);
      
      audioHandler
          .updateQueue(playlistState.currentMediaItems)
          .then((value) => {
                //播放音乐
                audioHandler.skipToQueueItem(index).then((value) => audioHandler.play()),
              });
